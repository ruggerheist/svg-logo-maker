...
describe('SVG', () => {
    it ('should return a string of the svg file data', () => {
       ...
       const svg = new SVG(data);
       fs.readFile('shape.svg', 'utf8', () => {
        expect(svg.render()).toEqual(`<polygon points="150, 18 244, 182 56, 182" fill='${data.shapeColor}' /><text x="150" y="125" font-size="60" text-anchor="middle" fill='${data.textColor}'>${data.text}</text></svg>`)
       })
    })
})
